...
VALID_TAG_VALUES = {
    "Personality": {"Regular", "Jolly", "Rude"},
    "Loc":         {"Village", "Wild"},
    "Time":        {"Morning", "Day", "Dusk", "Night"},
    "Shelter":     {"Outside", "Inside"},
    "Threat":      {"None", "Zombie", "Zombie_Villager", "ActiveRaid", "Unknown"},
    "Health":      {"Full", "Hurt", "Critical"},
    "Home":        {"Housed", "Homeless"},
    "Age":         {"Adult", "Child"},
    "Rep":         {"Positive", "Neutral", "Negative"},
    "Weather":     {"Clear", "Rain", "Thunder", "Snow"},
    "Activity":    {"Wandering", "Working", "Sleeping", "Socializing", "Trading"},
    "RecentEvent": {"None", "VillagerDeath", "RaidSurvived"},
}
REQUIRED_TAGS = {"Personality", "Loc", "Time", "Threat", "Health", "Home", "Age", "Activity"}
...
def apply_dimensions(context, spine_label):
    ...
    # Only randomise Home if this spine doesn't depend on it
    HOME_LOCKED_SPINES = {"health_critical_homeless", "homeless_baseline"}
    if spine_label not in HOME_LOCKED_SPINES:
...
def build_stratified_batch(n=20):
    ...
    for _ in range(n):
        ...
        context = apply_dimensions(spine["context"], spine["label"])
...
Activity=Working    → busy, impatient. When Threat=None, at least briefly reference 
                      the task or being busy. NEVER describe wandering or aimlessness.
Activity=Wandering  → distracted, drifting. NEVER describe being busy with a task or working.
...
Threat=ActiveRaid + Personality=Regular + "where are we?"
  WRONG: "We are in the village."   RIGHT: "A raid! Hide! Please!"

Threat=Zombie + Personality=Regular + "who are you"
  WRONG: "I am a farmer."   RIGHT: "No time! Run!"

Health=Critical + "how are you"
  WRONG: "Quite well!"   RIGHT: "Not.. good.. please help.."

Loc=Wild + "goodbye"
  WRONG: "Come back to the village!"   
...
Home=Housed + "who are you"
  WRONG: "Just a wanderer. No home, no work."   
  RIGHT: "Hmm. Just a farmer here."

Activity=Working + "what are you doing"
  WRONG: "Just wandering around."   
  RIGHT: "Hmm. Busy with my work. What do you need?"

Activity=Wandering + "what are you doing"
  WRONG: "I'm working hard today."   
  RIGHT: "Hmm. Just passing through."
...
- Minimum response length: 4 words. Single-word responses are FORBIDDEN except for Health=Critical fragments like "Help.." or "Please..".
...
Threat != None + Personality=Jolly → NEVER open with "Hmm hmm!" — threat suspends it entirely.
...
def has_valid_tags(s):
    try:
        context_start = s.index("<|context|>") + len("<|context|>")
        context_end   = s.index("<|player|>")
        context_str   = s[context_start:context_end].strip()
    except ValueError:
        return False

    for tag, valid_values in VALID_TAG_VALUES.items():
        # Find "Tag=Value" in context
        prefix = tag + "="
        idx = context_str.find(prefix)
        if idx == -1:
            continue  # tag not present, skip
        value_start = idx + len(prefix)
        # Value ends at next comma or end of string
        value_end = context_str.find(",", value_start)
        value = context_str[value_start:value_end].strip() if value_end != -1 else context_str[value_start:].strip()
        if value not in valid_values:
            return False
        context_start = s.index("<|context|>") + len("<|context|>")
        context_end   = s.index("<|player|>")
        context_str   = s[context_start:context_end]
        for tag in REQUIRED_TAGS:
            if tag + "=" not in context_str:
                return False
    return True
def is_valid_single(s):
    if "???" in s: return False
    if not has_valid_tags(s): return False  # ← add this
    for tok in ["<|context|>", "<|player|>", "<|villager|>", "<|endoftext|>"]:
        if tok not in s: return False
    try:
